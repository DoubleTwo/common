...
class CustomUserSegmenter:
    ...
    def _execute_sql(self, sql):
        ...
        try:
            res = urllib.request.urlopen("%s/sql/query?project=gaoyujie&%s" % (self._api_url, params),context=ssl._create_unverified_context())
            return 200, res
    ...
    def run(self):
        ...
        final_sql = '''
            SELECT
                DISTINCT COALESCE(cast(b.distinct_id as string), a.first_id), b.segment
            FROM users a
        ...
        for record in tmp_data_file_handler:
            ...
            record_row_num += 0
            if ( record_row_num % 1000 ) == 0 :
                print(" %d thousand lines were executed "  %record_row_num )
        sa.close()
